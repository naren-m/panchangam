...
func TestConvertSlogAttrToSpanAttr(t *testing.T) {
	...
	tests := []struct {
		...
		{"time", "key", slog.Time("key", timestamp), attribute.String("key", timestamp.String()), false},
		{"unsupported", "key", slog.Any("key", []byte("value")), attribute.String("key", "[118 97 108 117 101]"), false},
	}
